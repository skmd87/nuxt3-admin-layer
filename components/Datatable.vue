			...
			<template #top="{ }">
				<v-toolbar color="transparent" density="compact">
					...
					<slot name="toolbar.column-Visibility" v-bind="$bind.toolbar.columnVisibility">
						<v-btn v-bind="$bind.toolbar.columnVisibility">
							<v-menu activator="parent" close-on-back :close-on-content-click="false">
								<v-list
									v-model:selected="selectedHiddenColumns"
									density="compact"
									slim
									:items="$bind.toolbar.columnVisibility.items"
					...
					<slot name="toolbar.refresh" v-bind="$bind.toolbar.refresh">
						<v-btn v-bind="$bind.toolbar.refresh" />
					</slot>
				</v-toolbar>
			</template>
			<template v-for="(_, name) in ($slots as VDataTableServerSlots | {})" #[name]="slotData">
				<slot :name="name" v-bind="{ ...slotData }" />
			</template>
			<template v-if="!$bind.datatable.items?.length" #loading>
				<v-skeleton-loader :type="`table-row@${$p.itemsPerPage}`" />
			</template>
		</v-data-table-server>
	</api-items>
...
import ApiItems, { type ApiItemsProps } from './ApiItems.vue'
...
const $bind = reactive<{
	...
	datatable: {
		...
		headers: computed(() => {
			...
			return ($p.headers as Headers)?.filter(header => !selectedHiddenColumns.value.includes(header.value as string))
		}),
	...
	toolbar: {
		...
		columnVisibility: {
			items: computed(() => {
				const arr: any[] | undefined = ($p.headers as Headers)?.slice()
					.map(header => ({
						title: header.title,
						value: header.value,
					}))
